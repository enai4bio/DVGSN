...
class GIN(nn.Module):
    def __init__(self,
        ...
        for idx in range(self.layer_num):
            ...
            bn = nn.BatchNorm1d(in_size, momentum=self.momentum, track_running_stats=False)
...
class VirtualGIN(nn.Module):
    def __init__(self,
        ...
        self.gin_layer_list = nn.ModuleList()

        for idx in range(self.layer_num):
            in_size = mid_channels
            if idx == 0:
                in_size = self.in_channels
            mlp_layer = nn.Linear(in_size, mid_channels)
            self.linear_layer_list.append(mlp_layer)

            gin_layer = GIN(in_channels=in_size,
                            out_channels=self.out_channels,
                            layer_num=1)
            self.gin_layer_list.append(gin_layer)



    def forward(self, node_ft, adj):
        H = node_ft
        out = torch.zeros(self.node_num, self.out_channels).to(H.device)
        for idx in range(self.linear_layer_num):
            H = self.linear_layer_list[idx](H)
            H = self.activation_layer(H)
            out += self.gin_layer_list[idx](H, adj)

        out = self.mlp_activation(self.mlp_list[-1](H))
        return out
...
class VirtualGINTrainer(object):
    ...
    def train(self):
        ...
        for idx in range(train_size // self.batch_size):
            ...
            loss = loss + self.alpha * torch.norm(adj)
            loss.backward()
            self.optimizer.step()
...
if __name__ == '__main__':
    VirtualGINTrainer(wind_size=12, pred_step=1, data_type='us', seed=3,
                      layer_num=2).start(display=True)